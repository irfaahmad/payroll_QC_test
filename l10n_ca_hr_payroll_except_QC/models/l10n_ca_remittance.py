...
# All provincial/territorial Workers' Compensation board remittance types.
# Quebec's CNESST is included in anticipation of full Quebec support.
WCB_TYPES = (
    'wcb_ab', 'wcb_bc', 'wcb_mb', 'wcb_nb', 'wcb_nl', 'wcb_ns',
    'wcb_nt', 'wcb_nu', 'wcb_on', 'wcb_pe', 'wcb_qc', 'wcb_sk', 'wcb_yt',
)

# Liability account codes that belong to each remittance type.
# All WCB boards currently post to the shared 2354 account; split per-board
# if/when separate liability accounts are introduced.
...
class L10nCaRemittance(models.Model):
    ...
    remittance_type = fields.Selection(
        selection=[
            ...
            ('bc_eht',         'BC EHT'),
            # ---- Workers' Compensation — one entry per provincial/territorial board ----
            ('wcb_ab',  'WCB Alberta'),
            ('wcb_bc',  'WorkSafeBC (British Columbia)'),
            ('wcb_mb',  'WCB Manitoba'),
            ('wcb_nb',  'WorkSafeNB (New Brunswick)'),
            ('wcb_nl',  'WorkplaceNL (Newfoundland & Labrador)'),
            ('wcb_ns',  'WCB Nova Scotia'),
            ('wcb_nt',  'WSCC (Northwest Territories)'),
            ('wcb_nu',  'WSCC (Nunavut)'),
            ('wcb_on',  'WSIB (Ontario)'),
            ('wcb_pe',  'WCB Prince Edward Island'),
            ('wcb_qc',  'CNESST (Quebec)'),
            ('wcb_sk',  'WCB Saskatchewan'),
            ('wcb_yt',  'WCB Yukon'),
            # ---- Other deductions ----
    ...
    payslip_ids = fields.Many2many(
        ...
        'payslip_id',
        string='Payslips',
        domain="[('state', 'in', ['validated', 'paid']), ('company_id', '=', company_id)]",
    )
    ...
    def _compute_due_date(self):
        """Compute due date per CRA / provincial remittance schedules."""
        for rec in self:
    ...
    def _calc_due_date(self) -> date | bool:
        ...
        if rtype == 'cra_pd7a':
            if remitter_type == 'quarterly':
                ...
                # period_end + 3 business days (already a business day)
                return _add_business_days(period_end, 3)

            else:
                due = period_end + timedelta(days=15)

        elif rtype == 'provincial_eht':
            # Ontario EHT: March 15 of year following period_end.year
            due = date(period_end.year + 1, 3, 15)

        elif rtype == 'bc_eht':
            # BC EHT: March 31 of year following period_end.year
            due = date(period_end.year + 1, 3, 31)

        elif rtype in WCB_TYPES:
            # Default rule for every provincial WCB board: last day of the
            # month following quarter-end.  Refine per-board later if needed
            # (e.g. WSIB Ontario monthly schedules, WorkSafeBC by classification
            # unit reporting frequency, CNESST annual reconciliation).
            q_end_months = {3: 4, 6: 7, 9: 10, 12: 1}
    ...
    def action_confirm(self):
        ...
        for rec in self:
            ...
            if not rec.total_amount or rec.total_amount <= 0.01:
                raise UserError(
    ...
    def action_open_payment_wizard(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_amount': abs(self.total_amount),
            },
        }
    ...
    def _create_remittance_move(self):
        ...
        """
        # Prefer the dedicated remittance journal (REMIT); fall back to the
        # legacy SAL code, then to any general journal on the company.
        ...
        if not journal:
            raise UserError(_('No suitable journal found. Please configure a remittance journal (REMIT).'))
        ...
        move_lines.append((0, 0, {
            ...
            'name': _('Remittance — %s') % (self.name or ''),
            'debit': 0.0,
    ...
    def _create_pending_remittances(self, config, today=None):
        ...
        # CRA PD7A
        self._ensure_remittance(company, 'cra_pd7a', period_start, period_end)

        # Provincial EHT (annual — Ontario)
        year_start = date(today.year, 1, 1)
        year_end = date(today.year, 12, 31)
        self._ensure_remittance(company, 'provincial_eht', year_start, year_end)

        # WCB — provincial board determined by config.wcb_province (quarterly)
        wcb_province = (getattr(config, 'wcb_province', '') or '').lower()
        if wcb_province:
            wcb_key = f'wcb_{wcb_province}'
            if wcb_key in WCB_TYPES:
                q_start_month = ((today.month - 1) // 3) * 3 + 1
                q_end_month = q_start_month + 2
                last = calendar.monthrange(today.year, q_end_month)[1]
                self._ensure_remittance(
                    company, wcb_key,
                    date(today.year, q_start_month, 1),
                    date(today.year, q_end_month, last),
                )

        # RRSP — same period as payroll
        self._ensure_remittance(company, 'rrsp', period_start, period_end)

        # Union dues — same period as payroll
        self._ensure_remittance(company, 'union_dues', period_start, period_end)
