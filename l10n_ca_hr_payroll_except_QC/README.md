...
## WCB / CNESST Manual Rate Entry

### Where to find the menu

Navigate to **Payroll → Configuration → WCB / CNESST Rates**.

This menu is visible to users with the **Payroll Manager** role.

### How to configure rates

1. Click **Create** to open the rate entry form.
2. Select the **Province / Territory** that issued the assessment letter.
   - Quebec employers select **"Quebec (CNESST)"** — the same form is used for both WCB and CNESST.
3. Enter the **Board Name** (e.g. *WSIB*, *WorkSafeBC*, *CNESST*) and, optionally, your **Classification / Rate Group** code.
4. Enter the **Employer Rate (%)** exactly as shown on the assessment letter (e.g. `1.95` for 1.95%).
5. Enter the **Maximum Assessable Earnings** — the annual insurable earnings ceiling published by the board.
6. Set the **Effective From** date to the first day the rate applies.
7. Leave **Effective To** blank if the rate is currently in effect. Set it only when a superseding record is created.
8. Save the record.

> **Important — preserve history by adding new records:**  
> When your WCB rate changes, **create a new dated record** — never overwrite or edit the old one.  
> The payroll engine always looks up the rate whose *Effective From* is closest to (and not after) the payslip end date, so old payslips continue to reference the correct historical rate.

### Quebec (CNESST)

Quebec CNESST assessments work identically to provincial WCB assessments.  
Select **"Quebec (CNESST)"** as the province.  The same `WCB / CNESST (Employer)` salary rule handles the calculation for all provinces including QC.

### Exempt contracts

If a contract should not attract WCB / CNESST premiums (e.g. certain owner/executive officers), open the contract form and tick the **WCB Exempt** checkbox.  
The salary rule will produce a zero amount for that contract, regardless of any rate configured.
